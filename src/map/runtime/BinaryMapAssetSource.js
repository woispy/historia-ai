...
export class BinaryMapAssetSource {
  ...
  getProvinceGeometryRange(index,lod=0){if(index<0||index>=this.provinceCount)return null;if(lod!==0)return null;const b=index*LOD_STRIDE;return{tileOffset:this.lodRanges[b],tileCount:this.lodRanges[b+1]};}
...
function validateHeader(h,bytes){if(h.magic!==MAGIC)throw new Error("Invalid mapbin magic");if(h.version!==VERSION)throw new Error(`Unsupported mapbin version: ${h.version}`);if(h.totalByteLength!==bytes)throw new Error("Invalid mapbin length");if(h.lodRangeCount!==h.provinceCount)throw new Error("Invalid mapbin LOD layout: expected one LOD record per province");const provinceEnd=h.provinceOffset+h.provinceCount*PROVINCE_FIELD_COUNT*4,tileEnd=h.tileOffset+h.tileCount*TILE_STRIDE*4,geometryEnd=h.geometryOffset+h.geometryPointCount*8,lodEnd=h.lodOffset+h.lodRangeCount*LOD_STRIDE*4,cityEnd=h.cityOffset+h.cityCount*CITY_STRIDE*4,paletteEnd=h.paletteOffset+h.paletteByteLength;const offsets=[h.provinceOffset,h.tileOffset,h.geometryOffset,h.lodOffset,h.cityOffset,h.paletteOffset];if(offsets.some((o)=>o<HEADER_BYTES||o%4!==0||o>bytes)||provinceEnd>h.tileOffset||tileEnd>h.geometryOffset||geometryEnd>h.lodOffset||lodEnd>h.cityOffset||cityEnd>h.paletteOffset||paletteEnd!==bytes)throw new Error("Invalid mapbin section offsets");}
