...
// The authored font size keeps shrinking with zoom and bottoms out at a
// readable world-space value. screenScale then applies the remaining inverse
// zoom correction in the SVG so the actual on-screen size never grows again.
...
function getScreenStableWorldSize(city, zoom) {
  ...
  return (tierConfig(city).baseSize * SCREEN_STABLE_SIZE) / safeZoom;
}
...
export function getCityVisualStyle(city, zoom = 1) {
  ...
  const radius = clamp(config.markerRadius / safeZoom, 0.020, config.markerRadius * SCREEN_STABLE_SIZE);
...
function getCandidates(city, fontSize) {
  ...
  const dx = Number(city.map.labelDx ?? 0), dy = Number(city.map.labelDy ?? 0);
  const gap = Math.max(tier === "capital" ? 0.14 : 0.11, fontSize * 0.72);
  const side = Math.max(0.045, fontSize * 0.30), diagonal = gap * 0.82;
  return [
...
function isLabelInViewport(box, camera) {
  ...
  const zoom = Math.max(1, Number(camera.zoom) || 1), viewWidth = 360 / zoom, viewHeight = 180 / zoom;
  const centerX = Number(camera.x ?? 0), centerY = Number(camera.y ?? 0);
