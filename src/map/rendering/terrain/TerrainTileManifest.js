...
export function createTerrainTileManifest({ tile, bounds, assets }) {
  ...
  if (!assets?.heightmap || !assets?.normal || !assets?.splat || !assets?.landMask) {
    throw new Error("Terrain manifest requires heightmap, normal, splat and land-mask assets.");
  }
...
export function validateTerrainTileManifest(manifest) {
  ...
  return ["heightmap", "normal", "splat", "landMask"].every((key) => typeof manifest.assets[key] === "string");
}
