...
assert.deepEqual(cache.evictUnretained(new Set(["a", "c"])), []); assert.deepEqual(cache.evictUnretained(new Set(["a"])), ["c"]); assert.throws(() => cache.request("c"), /pending GPU eviction/); assert.equal(cache.finishEviction("c"), true); assert.equal(cache.snapshot().usedBytes, 40);
