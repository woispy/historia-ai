...
export function createTerrainHeightSampler({ bounds, width, height, samples, spacingX, spacingY } = {}) {
  ...
  function coordinates(lon, lat) { finite(lon, "longitude"); finite(lat, "latitude"); const u = clamp((lon - bounds.minX) / (bounds.maxX - bounds.minX), 0, 1); const v = clamp((lat - bounds.minY) / (bounds.maxY - bounds.minY), 0, 1); return { x: u * (width - 1), y: v * (height - 1), localX: u * spanX, localY: v * spanY }; }
