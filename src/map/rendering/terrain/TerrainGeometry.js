/** Build a terrain tile mesh with duplicated boundary vertices lowered by skirtDepth meters. */
export function buildTerrainGridMesh({ heights, size, skirtDepth = 50 }) {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    positions[p] = x / (size - 1); positions[p + 1] = y / (size - 1); vertexHeights[i] = heights[i];
  ...
  return Object.freeze({ positions, vertexHeights, uvs, normals, indices, skirtDepth });

  function copySkirtVertex(source, target) {
    positions[target * 2] = positions[source * 2]; positions[target * 2 + 1] = positions[source * 2 + 1];
    vertexHeights[target] = heights[source] - skirtDepth; uvs[target * 2] = uvs[source * 2]; uvs[target * 2 + 1] = uvs[source * 2 + 1];
