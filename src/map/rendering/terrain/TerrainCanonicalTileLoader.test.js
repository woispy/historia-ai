...
const identityMismatch = createCanonicalTerrainTileLoader({ manifestForTile: async () => manifest, fetchBinary: async () => new ArrayBuffer(4), decodeHeightmap: async () => ({ width:3,height:3,crs:"EPSG:4326",resolutionMeters:30,bounds:manifest.bounds,spacingX:30,spacingY:30,samples:new Float32Array(9) }) });
await assert.rejects(() => identityMismatch("3/10/21"), /identity mismatch/);
