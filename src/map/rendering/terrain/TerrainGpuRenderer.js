...
uniform sampler2D uDemValidity;
...
void main(){
  ...
  float tonal=mix(0.88,1.08,clamp(vHeight/5000.0,0.0,1.0));
...
export class TerrainGpuRenderer {
  constructor(gl, material = TERRAIN_MATERIAL_DEFAULTS) {
    ...
    this.gl = gl;
    this.material = { ...TERRAIN_MATERIAL_DEFAULTS, heightScale: 0.001, ...material };
    ...
    this.uniforms = {
      ...
      demValidity: gl.getUniformLocation(this.program, "uDemValidity"),
    };
  }
  ...
  uploadTile(tileId, asset, tileBounds = asset?.bounds, dataBounds = asset?.dataBounds) {
    ...
    const mesh = buildTerrainGridMesh({ heights: asset.heights, validity: asset.demValidity, size: asset.size, skirtDepth: 0 });
    ...
    const validityStats = measureValidityStats(asset.demValidity, asset.size);
    const neighborDeltaCount = countLargeNeighborDeltas(asset.heights, asset.demValidity, asset.size, LARGE_NEIGHBOR_DELTA_METERS);
    console.debug("[TerrainGpuRenderer] terrain upload telemetry", { tileId, version: asset.version, grid: asset.size, minHeight: uploadStats.min, maxHeight: uploadStats.max, finiteCount: uploadStats.finiteCount, invalidCount: validityStats.invalidCount, validCount: validityStats.validCount, validityCoverage: validityStats.coverage, neighborDeltaOver1000m: neighborDeltaCount, dataBounds: samplingBounds });
    const { gl } = this;
    const vao = gl.createVertexArray();
    const position = gl.createBuffer();
    const height = gl.createBuffer();
    ...
    this.tiles.set(tileId,{vao,buffers:[position,height,uv,index],textures,count:mesh.indices.length,bounds,dataBounds:samplingBounds,minHeight:heightRange.min,maxHeight:heightRange.max,skirtDepth:mesh.skirtDepth,validCount:validityStats.validCount,invalidCount:validityStats.invalidCount});
  }
...
function measureValidityStats(values,size){if(!(values instanceof Uint8Array)||values.length!==size*size)throw new Error("Terrain validity mask is missing or malformed.");let validCount=0;for(const value of values)if(value!==0)validCount+=1;const total=values.length;return{validCount,invalidCount:total-validCount,coverage:total>0?validCount/total:0};}
function countLargeNeighborDeltas(values,validity,size,threshold){let count=0;for(let y=0;y<size;y+=1)for(let x=0;x<size;x+=1){const index=y*size+x;if(!validity[index])continue;const current=values[index];if(x+1<size&&validity[index+1]&&Math.abs(current-values[index+1])>threshold)count+=1;if(y+1<size&&validity[index+size]&&Math.abs(current-values[index+size])>threshold)count+=1;}return count;}
