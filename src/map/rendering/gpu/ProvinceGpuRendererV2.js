...
layout(location=1) in uint a_province;
uniform vec2 u_camera;
uniform vec2 u_halfExtent;
flat out uint v_province;
...
void main(){ outColor = v_province==u_selected ? u_selectedColor : vec4(0.42,0.45,0.35,1.0); }`);
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, position); gl.enableVertexAttribArray(0); gl.vertexAttribPointer(0, 2, gl.FLOAT, false, 0, 0);
  gl.bindBuffer(gl.ARRAY_BUFFER, province); gl.enableVertexAttribArray(1); gl.vertexAttribIPointer(1, 1, gl.UNSIGNED_INT, 0, 0);
  gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, index);
  gl.bindVertexArray(null);
  const cameraLocation = gl.getUniformLocation(program, "u_camera");
  const halfExtentLocation = gl.getUniformLocation(program, "u_halfExtent");
  ...
  return {
    upload(nextPack) {
      if (!nextPack?.vertices || !nextPack?.indices || !nextPack?.provinces) throw new Error("Invalid indexed province pack");
      pack = nextPack;
      gl.bindBuffer(gl.ARRAY_BUFFER, position); gl.bufferData(gl.ARRAY_BUFFER, pack.vertices, gl.STATIC_DRAW);
      gl.bindBuffer(gl.ARRAY_BUFFER, province); gl.bufferData(gl.ARRAY_BUFFER, buildProvinceVertexIndices(pack), gl.STATIC_DRAW);
      gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, index); gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, pack.indices, gl.STATIC_DRAW);
      gl.bindVertexArray(null);
    },
    setVisibleRanges(ranges) { visibleRanges = Array.isArray(ranges) ? ranges : []; },
    render({ camera = { x: 0, y: 0 }, width = canvas.width, height = canvas.height, zoom = 1, selectedProvinceIndex = 0xffffffff } = {}) {
      if (!pack) return 0;
      const halfExtent = [180 / Math.max(zoom, 0.0001), 90 / Math.max(zoom, 0.0001)];
      gl.viewport(0, 0, canvas.width, canvas.height); gl.useProgram(program); gl.bindVertexArray(vao);
      gl.uniform2f(cameraLocation, Number(camera.x) || 0, Number(camera.y) || 0); gl.uniform2f(halfExtentLocation, halfExtent[0], halfExtent[1]);
      gl.uniform1ui(selectedLocation, Number(selectedProvinceIndex) >>> 0); gl.uniform4f(selectedColorLocation, 0.84, 0.69, 0.30, 1);
      let calls = 0;
      for (const range of visibleRanges) {
        const first = Number(range.firstIndex) || 0; const count = Number(range.indexCount) || 0;
        if (count > 0 && count % 3 === 0 && first >= 0 && first + count <= pack.indices.length) {
          gl.drawElements(gl.TRIANGLES, count, gl.UNSIGNED_INT, first * 4); calls += 1;
        }
      }
      gl.bindVertexArray(null); return calls;
    },
    dispose() { gl.deleteBuffer(position); gl.deleteBuffer(province); gl.deleteBuffer(index); gl.deleteVertexArray(vao); gl.deleteProgram(program); },
    gl,
  };
}

function buildProvinceVertexIndices(pack) {
  const out = new Uint32Array(pack.indices.length);
  for (let i = 0; i < pack.indices.length; i += 1) {
    const province = findProvinceForIndex(pack.provinces, i);
    out[i] = province;
  }
  return out;
}

function findProvinceForIndex(provinces, index) {
  for (const province of provinces) {
    for (const range of province.lodRanges ?? []) if (index >= range.firstIndex && index < range.firstIndex + range.indexCount) return province.provinceIndex;
  }
  return 0xffffffff;
}
