...
export function normalizeRing(ring) {
  ...
  while (changed && out.length > 3 && guard++ < out.length * 3) {
    ...
    for (let i = 0; i < out.length && out.length > 3; i += 1) {
      ...
      if (same(a, c) || squaredDistance(a, b) <= POSITION_EPSILON ** 2 || squaredDistance(b, c) <= POSITION_EPSILON ** 2) { out.splice(i, 1); changed = true; i -= 1; continue; }
      const scale = Math.max(1, Math.hypot(c[0] - a[0], c[1] - a[1]));
      if (Math.abs(cross(a, b, c)) <= COLLINEAR_EPSILON * scale) { out.splice(i, 1); changed = true; i -= 1; }
    }
  }
  return out;
}
function unwrapRing(points) {
  ...
  for (let i = 1; i < points.length; i += 1) {
    let longitude = points[i][0]; const previousLongitude = out[i - 1][0];
...
export function buildIndexedProvincePack(entries = [], options = {}) {
  ...
  if (!Number.isFinite(tileSize) || tileSize <= 0) throw new Error("Invalid tile size"); if (!Number.isFinite(quantization) || quantization <= 0) throw new Error("Invalid quantization");
  ...
  entries.forEach((entry, provinceIndex) => {
    ...
    onProgress?.({ phase: "province-start", provinceIndex, provinceId: id, provinceCount: entries.length, polygonCount: polygons.length, vertexCount: vertices.length / 2, indexCount: indices.length });
    ...
    onProgress?.({ phase: "province-complete", provinceIndex, provinceId: id, provinceCount: entries.length, polygonCount: polygons.length, vertexCount: vertices.length / 2, indexCount: indices.length });
  });
