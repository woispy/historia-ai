...
export function analyzeRing(ring) {
    ...
    outer: for (let i = 0; i < n; i += 1) {
      for (let j = i + 1; j < n; j += 1) {
        ...
        if (segmentsIntersect(points[i], points[(i + 1) % n], points[j], points[(j + 1) % n])) {
          ...
          if (intersections.length >= MAX_DIAGNOSTIC_INTERSECTIONS) break outer;
        }
      }
    }
  }
  ...
  return Object.freeze({
    rawVertexCount: Array.isArray(ring) ? ring.length : 0,
    normalizedVertexCount: normalized.length,
    signedArea: area,
    longitudeSpan: points.length ? Math.max(...points.map((p) => p[0])) - Math.min(...points.map((p) => p[0])) : 0,
    selfIntersections: Object.freeze(intersections),
    selfIntersectionCheck: n <= 2048 ? "complete" : "bounded-skipped",
    finite: points.every((p) => Number.isFinite(p[0]) && Number.isFinite(p[1])),
    simple,
    triangulable: points.length >= 3 && simple && Math.abs(area) > EPSILON,
    reason: points.length < 3 ? "insufficient-vertices" : !simple ? "self-intersection" : Math.abs(area) <= EPSILON ? "zero-area" : "ok",
  });
}
...
export function triangulateRing(ring, context = {}) {
  ...
  if (points.length > MAX_TRIANGULATION_VERTICES) {
    throw new Error(`Province triangulation exceeds safety bound${context.provinceId ? ` province=${context.provinceId}` : ""}; vertices=${points.length}`);
  }
  ...
  if (ids.length === 3 && Math.abs(cross(points[ids[0]], points[ids[1]], points[ids[2]])) > EPSILON) {
...
function updateBounds(bounds, ring) {
...
export function buildIndexedProvincePack(entries = [], options = {}) {
  ...
  entries.forEach((entry, provinceIndex) => {
    ...
    for (let lod = 0; lod < 4; lod += 1) {
      ...
      for (let polygonIndex = 0; polygonIndex < lods.length; polygonIndex += 1) {
        ...
        const triangleIndices = diagnostics.triangulable ? triangulateRing(ring, { provinceId, polygonIndex, lod }) : [];
    ...
    const geometryStatus = renderable ? "renderable" : "non-renderable";
    provinces.push(Object.freeze({
      provinceIndex,
      provinceId,
    ...
    onProgress?.({ phase: "province-complete", provinceIndex, provinceId, provinceCount: entries.length, polygonCount: polygons.length, vertexCount: vertices.length / 2, indexCount: indices.length, geometryStatus, nonRenderableReason: renderable ? null : firstNonRenderableReason });
  });
  ...
  const renderableProvinceCount = provinces.filter((province) => province.renderable).length;
  const nonRenderableProvinceCount = provinces.length - renderableProvinceCount;
  onProgress?.({ phase: "pack-complete", provinceCount: provinces.length, renderableProvinceCount, nonRenderableProvinceCount, tileCount: tiles.size, vertexCount: vertices.length / 2, indexCount: indices.length });
