...
fn cull(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  let b=province*4u;
  let minX=bounds[b];let minY=bounds[b+1u];let maxX=bounds[b+2u];let maxY=bounds[b+3u];
  let visible=maxX>=camera.center.x-camera.halfExtent.x&&minX<=camera.center.x+camera.halfExtent.x&&maxY>=camera.center.y-camera.halfExtent.y&&minY<=camera.center.y+camera.halfExtent.y;
  let lodHint=lods[tileIndex*4u+1u];
  let outBase=tileIndex*4u;
  indirect[outBase]=tiles[tile];
  indirect[outBase+1u]=select(0u,tiles[tile+1u],visible&&lodHint>=0u);
  indirect[outBase+2u]=0u;indirect[outBase+3u]=0u;
}
`;

export class WebGPUMapRenderer extends MapRendererContract {
  constructor(canvas){super();this.canvas=canvas;this.device=null;this.context=null;this.format=null;this.pipeline=null;this.cullBindGroup=null;this.buffers=null;this.assetSource=null;this.destroyed=false;this.frameRequest=0;this.camera={x:0,y:0,zoom:1,pitch:24,yaw:0};this.selected=0;this.hovered=0;}
  static isSupported(){return typeof navigator!=="undefined"&&Boolean(navigator.gpu);}
  async initialize({assetSource}={}){if(this.destroyed)throw new Error("Cannot initialize a disposed WebGPU renderer");if(!assetSource||!WebGPUMapRenderer.isSupported())return false;const adapter=await navigator.gpu.requestAdapter({powerPreference:"high-performance"});if(!adapter)return false;this.device=await adapter.requestDevice();this.context=this.canvas.getContext("webgpu");if(!this.context){this.dispose();return false;}this.format=navigator.gpu.getPreferredCanvasFormat();this.context.configure({device:this.device,format:this.format,alphaMode:"opaque"});this.assetSource=assetSource;this.buffers=this.createAssetBuffers(assetSource);this.createCullingPipeline(assetSource);return true;}
  createAssetBuffers(source){
    ...
    const indirect=this.device.createBuffer({size:Math.max(16,source.tileCount*16),usage:GPUBufferUsage.STORAGE|GPUBufferUsage.INDIRECT|GPUBufferUsage.COPY_DST});
  ...
  createCullingPipeline(source){
    const d=this.device,module=d.createShaderModule({code:CULL_WGSL}),cameraBuffer=d.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});
    ...
    this.cullBindGroup=d.createBindGroup({layout,entries:[{binding:0,resource:{buffer:this.buffers.tiles}},{binding:1,resource:{buffer:this.buffers.lods}},{binding:2,resource:{buffer:this.buffers.bounds}},{binding:3,resource:{buffer:this.buffers.indirect}},{binding:4,resource:{buffer:cameraBuffer}}]});
  ...
  render(){if(this.destroyed||!this.device||!this.cullBindGroup)return;const z=Math.max(.001,Number(this.camera.zoom)||1);this.device.queue.writeBuffer(this.buffers.camera,0,new Float32Array([Number(this.camera.x)||0,Number(this.camera.y)||0,180/z,90/z]));const encoder=this.device.createCommandEncoder();const pass=encoder.beginComputePass();pass.setPipeline(this.pipeline);pass.setBindGroup(0,this.cullBindGroup);pass.dispatchWorkgroups(Math.ceil(this.tileCount/64));pass.end();this.device.queue.submit([encoder.finish()]);}
  ...
  dispose(){if(this.destroyed)return;this.destroyed=true;this.stop();for(const buffer of Object.values(this.buffers||{})){if(buffer&&typeof buffer.destroy==="function")buffer.destroy();}this.buffers=null;this.cullBindGroup=null;this.pipeline=null;this.assetSource=null;this.context=null;this.device?.destroy?.();this.device=null;}
}
export { CULL_WGSL };
