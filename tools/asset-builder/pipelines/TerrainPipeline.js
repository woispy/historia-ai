...
async function sampleTile(source, bounds, size) {
  ...
  for (let y = 0; y < size; y += 1) { const lat = bounds.maxY - (bounds.maxY - bounds.minY) * y / (size - 1); for (let x = 0; x < size; x += 1) { const lon = bounds.minX + (bounds.maxX - bounds.minX) * x / (size - 1), keyLat = Math.floor(lat === bounds.maxY ? lat - 1e-9 : lat), keyLon = Math.floor(lon === bounds.maxX ? lon - 1e-9 : lon), key = `${keyLat}/${keyLon}`; if (!cache.has(key)) cache.set(key, await source.readTile(keyLat, keyLon)); const value = sampleCopernicusRaster(cache.get(key), lon, lat), height = Number.isFinite(value) ? value : 0; heights[y * size + x] = height; if (Number.isFinite(value)) { valid[y * size + x] = 255; min = Math.min(min, height); max = Math.max(max, height); } } }
