...
function pointOnSegment(point, start, end) {
  ...
  return point[0] >= Math.min(start[0], end[0]) - EPS && point[0] <= Math.max(start[0], end[0]) + EPS
    && point[1] >= Math.min(start[1], end[1]) - EPS && point[1] <= Math.max(start[1], end[1]) + EPS;
}
...
function isLakeShorelinePoint(point) {
  return LAKES.some((lake) => lake.coordinates.some((start, index) => pointToSegmentDistance(point, start, lake.coordinates[(index + 1) % lake.coordinates.length]) <= LAKE_SHORELINE_TOLERANCE));
}
...
function nearestLandPoint(point) {
  ...
  for (const polygon of LAND_POLYGONS) for (let index = 0; index < polygon.length; index += 1) {
    ...
    const dx = end[0] - start[0]; const dy = end[1] - start[1]; const denominator = dx * dx + dy * dy;
    const t = denominator < EPS ? 0 : Math.max(0, Math.min(1, ((point[0] - start[0]) * dx + (point[1] - start[1]) * dy) / denominator));
    const candidate = [start[0] + dx * t, start[1] + dy * t]; const distance = Math.hypot(point[0] - candidate[0], point[1] - candidate[1]);
...
function addBoundarySites(sites, polygon, kind) {
  for (let index = 0; index < polygon.length - 1; index += 1) {
    const start = polygon[index]; const end = polygon[index + 1];
    const length = Math.hypot(end[0] - start[0], end[1] - start[1]); const steps = Math.max(1, Math.ceil(length / BOUNDARY_SAMPLE_STEP));
    for (let step = 0; step <= steps; step += 1) { const t = step / steps; sites.push({ point: [start[0] + (end[0] - start[0]) * t, start[1] + (end[1] - start[1]) * t], provinceId: null, kind }); }
  }
}
function buildPhysicalBoundarySites() {
  ...
  for (const polygon of PHYSICAL_CORRECTION_POLYGONS) addBoundarySites(sites, polygon, "physical-coast-correction-boundary");
...
function halfPlane(polygon, a, b, c) {
  ...
  for (let index = 0; index < polygon.length; index += 1) {
    const current = polygon[index]; const next = polygon[(index + 1) % polygon.length]; const currentInside = inside(current); const nextInside = inside(next);
    if (currentInside && nextInside) output.push(next);
    else if (currentInside !== nextInside) {
      const currentValue = a * current[0] + b * current[1] - c; const nextValue = a * next[0] + b * next[1] - c; const denominator = currentValue - nextValue;
  ...
  for (let edge = 0; edge < clip.length; edge += 1) {
    ...
    for (let index = 0; index < input.length; index += 1) {
      ...
      if (currentInside && nextInside) output.push(next);
        ...
        const currentValue = cross(start, end, current); const nextValue = cross(start, end, next); const denominator = currentValue - nextValue; const t = Math.abs(denominator) < EPS ? 0 : currentValue / denominator;
        output.push([current[0] + (next[0] - current[0]) * t, current[1] + (next[1] - current[1]) * t]);
        if (!currentInside && nextInside) output.push(next);
      }
    }
...
function segmentCrossesLakeInterior(start, end) {
  const length = Math.hypot(end[0] - start[0], end[1] - start[1]);
  const steps = Math.max(2, Math.ceil(length / EDGE_SAMPLE_STEP));
  for (let step = 0; step <= steps; step += 1) {
    const t = step / steps; const point = [start[0] + (end[0] - start[0]) * t, start[1] + (end[1] - start[1]) * t];
    if (inLake(point) && !isLakeShorelinePoint(point)) return true;
  }
  return false;
}
function edgeOnPhysicalLand(polygon) {
  for (let index = 0; index < polygon.length; index += 1) {
    const start = polygon[index]; const end = polygon[(index + 1) % polygon.length];
    if (segmentCrossesLakeInterior(start, end)) return false;
    const length = Math.hypot(end[0] - start[0], end[1] - start[1]); const steps = Math.max(1, Math.ceil(length / EDGE_SAMPLE_STEP));
    for (let step = 0; step <= steps; step += 1) {
      const t = step / steps; const point = [start[0] + (end[0] - start[0]) * t, start[1] + (end[1] - start[1]) * t];
      if (isPhysicalLandPoint(point) || isLakeShorelinePoint(point)) continue;
      return false;
    }
  }
  return true;
...
function buildControlSites() {
  return ANATOLIA_PROVINCE_METADATA.map((item) => { const historicalAnchor = rawAnchor(item); return { point: nearestLandPoint(historicalAnchor), historicalAnchor, provinceId: item.id, kind: "province-anchor" }; });
}
...
function featureWeightBias() {
  ...
  for (const feature of [...ANATOLIA_STRATEGIC_PASSES, ...ANATOLIA_RIVER_CROSSINGS]) for (const provinceId of feature.provinces ?? []) if (provinceId in bias) bias[provinceId] += FEATURE_WEIGHT_STEP;
  for (const item of ANATOLIA_PROVINCE_METADATA) {
    const terrainClass = ANATOLIA_PROVINCE_REFINEMENTS[item.id]?.terrainClass;
    if (terrainClass === "lake-mountain" || terrainClass === "highland-lake" || terrainClass === "lake-basin") bias[item.id] += LAKE_TERRAIN_WEIGHT_BIAS;
  }
  return bias;
}
function buildPartition(sites, weights) {
  ...
  for (let index = 0; index < sites.length; index += 1) {
    ...
    const site = sites[index]; const cell = powerCell(index, sites, weights);
...
function solveWeights(sites) {
  ...
  for (let iteration = 0; iteration < MAX_WEIGHT_ITERATIONS; iteration += 1) {
    const summary = [...partition.entries()].map(([id, polygon]) => ({ id, area: area(polygon) })); const medianArea = median(summary.map((item) => item.area)); const proposedWeights = { ...weights };
    ...
    let next = null; let acceptedWeights = null; let scale = 1;
    for (let attempt = 0; attempt < WEIGHT_BACKOFF_ATTEMPTS; attempt += 1) {
      const candidateWeights = { ...weights };
      for (const item of summary) candidateWeights[item.id] = (weights[item.id] ?? 0) + ((proposedWeights[item.id] ?? weights[item.id] ?? 0) - (weights[item.id] ?? 0)) * scale;
      try { next = buildPartition(sites, candidateWeights); acceptedWeights = candidateWeights; break; }
      catch (error) {
        if (!String(error?.message ?? error).includes("empty power cell") && !String(error?.message ?? error).includes("invalid physical-land geometry")) throw error;
        scale *= 0.5;
      }
    }
    if (!next || !acceptedWeights) break;
    const maxDelta = Math.max(...summary.map((item) => Math.abs(area(next.get(item.id)) - item.area)));
    Object.assign(weights, acceptedWeights); partition = next;
    if (maxDelta < 0.0005) break;
  }
  return partition;
}
function manifestGeometry(provinceId) { const key = ANATOLIA_1300_PROVINCE_GEOMETRY_KEYS[provinceId]; return key ? ANATOLIA_1300_PROVINCE_GEOMETRY_MANIFEST[key] : null; }
function buildProvinceGeometry(item, polygon, site) {
  const historicalAnchor = site.historicalAnchor; const manifest = manifestGeometry(item.id);
  const identity = { id: item.id, provinceId: item.id, name: item.name, historicalAnchor, classification: "phase2d-anatolia-province-geometry", sourceFeatureId: item.id };
  const holeRings = [];
  for (const lake of LAKES) {
    const clippedLake = clipLandByCell(lake.coordinates, polygon);
    if (clippedLake.length >= 3 && area(clippedLake) >= MIN_AREA && pointInPolygon(lake.coordinates[0], polygon)) holeRings.push(clippedLake);
  }
  return { type: "Feature", geometry: { type: "Polygon", coordinates: [polygon, ...holeRings] }, identity, properties: { provinceId: item.id, provinceName: item.name, historicalAnchor, borderConfidence: item.borderConfidence, manifest, cartographicCentroid: polygonCentroid(polygon) } };
}
export function buildAnatoliaPhase2DAssets(regions) {
  const sites = buildControlSites(); const partition = solveWeights(sites);
  const geometries = ANATOLIA_PROVINCE_METADATA.map((item) => {
    const site = sites.find((candidate) => candidate.provinceId === item.id); const polygon = partition.get(item.id);
  ...
  const physicalSites = buildPhysicalBoundarySites(); const siteCount = sites.length + physicalSites.length;
  ...
  return { version: 16, historicalDate: "1300-01-01", regions, geometries, provinces: geometries, siteCount, barrierSiteCount: 0, politicalSiteCount: sites.length, fallbackProvinceCount, polygonCount, vertexCount, source: "curated-voronoi-power-cells-clipped-to-physical-land" };
}
export { isPhysicalLandPoint, isAnatoliaGeometryPoint };
function isAnatoliaGeometryPoint(point) { return Array.isArray(point) && point.length === 2 && point[0] >= BBOX[0] && point[0] <= BBOX[2] && point[1] >= BBOX[1] && point[1] <= BBOX[3]; }
