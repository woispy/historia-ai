...
function nearestLandPoint(point) {
  ...
  if (!best || bestDistance > MAX_ANCHOR_SNAP_DISTANCE) {
    throw new Error(`Historical province anchor is too far from physical land: ${point.join(",")} (${bestDistance.toFixed(3)}°)`);
  }
...
function buildPhysicalBoundarySites() {
  const sites = [];
  for (const polygon of LAND_POLYGONS) addBoundarySites(sites, polygon, "physical-land-boundary");
  ...
  for (const sea of ANATOLIA_PHYSICAL_ATLAS.seas) {
    if (sea.coordinates?.length >= 3) addBoundarySites(sites, sea.coordinates, "sea-boundary");
  }
  return sites;
}
...
function powerCell(index, sites, weights) {
  ...
  for (let otherIndex = 0; otherIndex < sites.length; otherIndex += 1) {
    ...
    const otherWeight = weights[sites[otherIndex].provinceId] ?? 0;
    polygon = halfPlane(
      polygon,
      2 * (other[0] - site[0]),
      2 * (other[1] - site[1]),
      other[0] ** 2 + other[1] ** 2 - site[0] ** 2 - site[1] ** 2 + ownWeight - otherWeight,
    );
...
function featureWeightBias() {
  ...
  for (const feature of [...ANATOLIA_STRATEGIC_PASSES, ...ANATOLIA_RIVER_CROSSINGS]) {
    for (const provinceId of feature.provinces ?? []) {
      if (provinceId in bias) bias[provinceId] += FEATURE_WEIGHT_STEP;
    }
  }
  return bias;
}
...
function buildPartition(sites, weights) {
  ...
  for (let index = 0; index < sites.length; index += 1) {
    ...
    if (!cell.length) throw new Error(`Phase 2D V14 empty power cell: ${site.provinceId}`);
    const polygon = clipCellToLand(cell, site.point)[0];
    if (!polygon || !edgeOnPhysicalLand(polygon)) {
      throw new Error(`Phase 2D V14 produced invalid physical-land geometry: ${site.provinceId}`);
    }
...
function solveWeights(sites) {
  ...
  const weights = Object.fromEntries(sites.filter((site) => site.provinceId).map((site) => [
    site.provinceId,
    featureBias[site.provinceId] ?? 0,
  ]));
  ...
  if (maxArea > medianArea * MAX_AREA_RATIO) {
    throw new Error(`Phase 2D V14 could not bound province area ratio: max ${maxArea.toFixed(3)} vs median ${medianArea.toFixed(3)}`);
  }
...
function headers(item, type) {
  return {
    assetType: type,
    assetVersion: 15,
    generator: "Historia AI Phase 2D Geometry Builder V14",
...
function provinceAsset(item, polygon, holes, site) {
  return {
    ...
    historical: {
      ...
      precision: "single-anchor-weighted-land-partition-with-physical-boundary-barriers-and-lake-holes",
      anchor: site.point,
      historicalAnchor: site.historicalAnchor,
...
export function buildAnatoliaPhase2DAssets() {
  ...
  for (const site of controlSites) {
    if (!isAnatoliaGeometryPoint(site.point) || !isPhysicalLandPoint(site.point)) {
      throw new Error(`Invalid snapped 1300 province anchor: ${site.provinceId} ${site.point.join(",")}`);
    }
  }
  ...
  for (const item of ANATOLIA_PROVINCE_METADATA) {
    ...
    if (!polygon) throw new Error(`Phase 2D V14 produced no geometry for ${item.id}`);
  ...
  return {
    ...
    method: "38 historical province identities, preserved historical anchors, deterministic weighted power partition constrained by physical land/coast/lake boundary sites, explicit lake holes, no political support fragments",
    siteCount: physicalSamplingSiteCount + controlSites.length + boundarySites.length + naturalFeatureSiteCount,
    politicalSiteCount: controlSites.length,
    physicalSamplingSiteCount,
    barrierSiteCount: boundarySites.length,
