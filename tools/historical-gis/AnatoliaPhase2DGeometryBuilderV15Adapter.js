...
import { repairPhysicalPolygon } from "./recovery/physical-edge-repair-v3.mjs";
...
function physicalFailureDetail(polygon) {
  ...
  for (let index = 0; index < polygon.length; index += 1) {
    ...
    for (let sampleIndex = 0; sampleIndex <= 128; sampleIndex += 1) {
      ...
      const point = [start[0] + (end[0] - start[0]) * fraction, start[1] + (end[1] - start[1]) * fraction];
      if ((!isPhysicalLandPoint(start) && !isFinalPhysicalGeometryBoundaryPoint(start)) || (!isPhysicalLandPoint(end) && !isFinalPhysicalGeometryBoundaryPoint(end)) || (!isPhysicalLandPoint(point) && !isFinalPhysicalGeometryBoundaryPoint(point))) return `edge=${index}; fraction=${fraction.toFixed(6)}; point=${point.map((value) => Number(value.toFixed(10))).join(",")}; start=${start.join(",")}; end=${end.join(",")}`;
    }
  }
...
function isStrictlyPhysicalOpenPath(path) {
  ...
  for (let index = 0; index < path.length - 1; index += 1) {
    ...
    if ((!isPhysicalLandPoint(start) && !isFinalPhysicalGeometryBoundaryPoint(start)) || (!isPhysicalLandPoint(end) && !isFinalPhysicalGeometryBoundaryPoint(end))) return false;
...
function pointInPolygon(point, polygon) {
  ...
  for (let index = 0, previous = polygon.length - 1; index < polygon.length; previous = index++) {
    ...
    if (Math.abs(cross) <= PARTITION_CLIP_EPS && point[0] >= Math.min(before[0], current[0]) - PARTITION_CLIP_EPS && point[0] <= Math.max(before[0], current[0]) + PARTITION_CLIP_EPS && point[1] >= Math.min(before[1], current[1]) - PARTITION_CLIP_EPS && point[1] <= Math.max(before[1], current[1]) + PARTITION_CLIP_EPS) return true;
    if ((current[1] > point[1]) !== (before[1] > point[1]) && point[0] < ((before[0] - current[0]) * (point[1] - current[1])) / ((before[1] - current[1]) || Number.EPSILON) + current[0]) inside = !inside;
  }
  return inside;
}
...
function dedupePolygon(polygon) {
  const result = [];
  for (const point of polygon) { const previous = result[result.length - 1]; if (!previous || Math.hypot(previous[0] - point[0], previous[1] - point[1]) > PARTITION_CLIP_EPS) result.push([...point]); }
  if (result.length > 1) { const first = result[0]; const last = result[result.length - 1]; if (Math.hypot(first[0] - last[0], first[1] - last[1]) <= PARTITION_CLIP_EPS) result.pop(); }
  return result;
}
...
function clipConvexPolygon(polygon, cell) {
  if (!cell) return polygon; let output = dedupePolygon(polygon); const clip = signedArea(cell) < 0 ? [...cell].reverse() : cell;
  for (let edgeIndex = 0; edgeIndex < clip.length; edgeIndex += 1) {
    if (output.length < 3) return []; const start = clip[edgeIndex]; const end = clip[(edgeIndex + 1) % clip.length]; const input = output; output = [];
    ...
    for (let index = 0; index < input.length; index += 1) {
      ...
      else if (currentInside !== nextInside) { const currentValue = edgeCross(start, end, current); const nextValue = edgeCross(start, end, next); const denominator = currentValue - nextValue; const fraction = Math.abs(denominator) <= PARTITION_CLIP_EPS ? 0 : currentValue / denominator; output.push([current[0] + (next[0] - current[0]) * fraction, current[1] + (next[1] - current[1]) * fraction]); if (!currentInside && nextInside) output.push(next); }
    }
  }
...
function sampledPhysicalBoundaryFallback(polygon) {
  ...
  for (let index = 0; index < polygon.length; index += 1) {
    ...
    for (let sampleIndex = 0; sampleIndex < PHYSICAL_BOUNDARY_FALLBACK_SAMPLES; sampleIndex += 1) {
      if (index > 0 && sampleIndex === 0) continue; const fraction = sampleIndex / PHYSICAL_BOUNDARY_FALLBACK_SAMPLES; const original = [start[0] + (end[0] - start[0]) * fraction, start[1] + (end[1] - start[1]) * fraction];
      const resolved = isPhysicalLandPoint(original) || isFinalPhysicalGeometryBoundaryPoint(original) ? original : resolvePhysicalGeometryBoundaryPoint(original);
      if (!resolved || (!isPhysicalLandPoint(resolved) && !isFinalPhysicalGeometryBoundaryPoint(resolved))) return null;
      const previous = result[result.length - 1]; if (!previous || Math.hypot(previous[0] - resolved[0], previous[1] - resolved[1]) > PARTITION_CLIP_EPS) result.push([...resolved]);
    }
  }
...
function repairPhysicalPolygonToFixedPoint(polygon, provinceId, containmentPolygon) {
  ...
  for (let pass = 1; pass <= MAX_PHYSICAL_REPAIR_PASSES; pass += 1) {
    const currentSignature = polygonSignature(current); if (currentSignature === previousSignature) break; previousSignature = currentSignature;
    try {
      const repaired = pass === 1 || !containmentPolygon ? repairPhysicalPolygon(current) : repairPhysicalPolygon(current, { containmentPolygon }); const normalized = dedupePolygon(repaired);
      ...
      if (containmentPolygon && isStrictlyPhysicalPath(normalized)) { const clipped = clipConvexPolygon(normalized, containmentPolygon); if (clipped.length >= 3 && isStrictlyPhysicalPath(clipped) && polygonInsideCell(clipped, containmentPolygon)) return clipped; current = clipped.length >= 3 ? clipped : normalized; continue; }
      current = normalized;
    } catch (error) {
      lastError = error; const sampled = sampledPhysicalBoundaryFallback(current);
      if (sampled) {
        if (isStrictlyPhysicalPath(sampled) && polygonInsideCell(sampled, containmentPolygon)) return sampled;
        if (containmentPolygon) { const clipped = clipConvexPolygon(sampled, containmentPolygon); if (clipped.length >= 3 && isStrictlyPhysicalPath(clipped) && polygonInsideCell(clipped, containmentPolygon)) return clipped; if (clipped.length >= 3) current = clipped; }
        else current = sampled;
      }
      if (!containmentPolygon) break;
      try { const unconstrained = dedupePolygon(repairPhysicalPolygon(current)); const clipped = clipConvexPolygon(unconstrained, containmentPolygon); if (clipped.length >= 3 && isStrictlyPhysicalPath(clipped) && polygonInsideCell(clipped, containmentPolygon)) return clipped; if (clipped.length >= 3) current = clipped; }
      catch (fallbackError) { lastError = fallbackError; break; }
    }
  }
  const detail = Array.isArray(current) ? polygonSignature(current) : "unavailable"; throw new Error(`Phase 2D physical repair failed for ${provinceId}: ${lastError?.message ?? "did not converge"}; physicalFailure=${physicalFailureDetail(current)}; polygon=${detail}`);
}
function normalizeGeometryContract(assets) {
  return { ...assets,
    ...
    geometries: assets.geometries.map((geometry) => {
      const polygon = geometry.polygons?.[0]; const holes = geometry.holes ?? []; if (!Array.isArray(polygon) || polygon.length < 3) return geometry;
      const provinceId = geometry.identity?.provinceId ?? geometry.identity?.id; const historicalAnchor = provinceId ? ANATOLIA_PROVINCE_REFINEMENTS[provinceId]?.anchor : null;
      if (!historicalAnchor) throw new Error(`Missing historical anchor in V16 adapter contract: ${provinceId ?? "unknown"}`);
      const sourcePartitionCell = geometry.sourcePartitionCell; const repairedPolygon = repairPhysicalPolygonToFixedPoint(polygon, provinceId ?? "unknown", sourcePartitionCell);
