...
const MAX_REPAIR_ITERATIONS = 28;
const ANCHOR_SEARCH_STEP = 0.01;
const ANCHOR_SEARCH_RADIUS = 0.30;
...
function polygonCentroid(polygon) {
  ...
  if (Math.abs(signed) < EPS) {
    const sum = polygon.reduce((t, p) => [t[0] + p[0], t[1] + p[1]], [0, 0]);
...
function isCorrectionShorelineVertex(point) {
  return correctionLandPolygons().some((polygon) => polygon.some((vertex) => pointOnSegment(point, vertex, vertex)));
}

function isAtlasLandBoundaryPoint(point) {
  return ANATOLIA_PHYSICAL_ATLAS.landPolygons.some((polygon) => polygon.some((vertex, index) => pointOnSegment(point, vertex, polygon[(index + 1) % polygon.length])));
}
...
export function isPhysicalLandPoint(point) {
  ...
  if (isExplicitExcludedWater(point) && !isExplicitLandControlPoint(point)) return false;
  if (isExplicitLandControlPoint(point)) return true;
  if (inCorrectionLandPatch(point) || isCorrectionShorelineVertex(point) || isAtlasLandBoundaryPoint(point)) return true;
  ...
  for (const polygon of ANATOLIA_PHYSICAL_ATLAS.landPolygons) for (let i = 0; i < polygon.length; i += 1) distance = Math.min(distance, distanceToSegment(point, polygon[i], polygon[(i + 1) % polygon.length]));
...
function physicalRepresentative(polygon) {
  return [polygonCentroid(polygon), ...polygon].find((candidate) => isPhysicalLandPoint(candidate)) ?? null;
}
...
function resolvePhysicalAnchor(point) {
  ...
  for (const polygon of [...ANATOLIA_PHYSICAL_ATLAS.landPolygons, ...correctionLandPolygons()]) {
    for (let index = 0; index < polygon.length; index += 1) {
      ...
      if (distance < bestDistance && isPhysicalLandPoint(candidate)) {
        boundaryBest = candidate;
        bestDistance = distance;
      }
    }
  }
  return boundaryBest ?? point;
}
...
function powerCell(index, sites, weights = {}) {
  ...
  for (let other = 0; other < sites.length; other += 1) {
    ...
    const candidate = halfPlane(polygon, 2 * (point[0] - site[0]), 2 * (point[1] - site[1]), point[0] ** 2 + point[1] ** 2 - site[0] ** 2 - site[1] ** 2 + weight - otherWeight);
...
function polygonIsSafe(polygon) {
  return polygon.length >= 3 && area(polygon) >= MIN_AREA && Boolean(physicalRepresentative(polygon)) && edgeOnPhysicalLand(polygon) && polygon.every((point) => isPhysicalLandPoint(point));
}

function repairPolygon(polygon, anchorPoint) {
  if (polygonIsSafe(polygon)) return polygon;
  const center = [anchorPoint, polygonCentroid(polygon), ...polygon].find((point) => isPhysicalLandPoint(point) && pointInPolygon(point, polygon)) ?? anchorPoint;
  let result = polygon;
...
function clipCellToPhysicalLand(cell, anchorPoint) {
  const primary = ANATOLIA_PHYSICAL_ATLAS.landPolygons.map((land) => clipPolygon(land, cell)).filter((polygon) => polygon.length >= 3 && area(polygon) >= MIN_AREA);
  const anchored = primary.filter((polygon) => pointInPolygon(anchorPoint, polygon));
  const sources = anchored.length
    ? anchored
    : [
        ...primary,
        ...correctionLandPolygons().map((land) => clipPolygon(land, cell)).filter((polygon) => polygon.length >= 3 && area(polygon) >= MIN_AREA && pointInPolygon(anchorPoint, polygon)),
      ];
  return sources.map((polygon) => repairPolygon(polygon, anchorPoint)).filter((polygon) => polygon.length >= 3 && area(polygon) >= MIN_AREA && polygonIsSafe(polygon));
}
...
function buildPartition(sites, weights) {
  ...
  for (let index = 0; index < sites.length; index += 1) {
    ...
    if (!polygons.length) {
      cell = powerCell(index, sites, {});
      polygons = clipCellToPhysicalLand(cell, site.point);
    }
...
function filterPhysicalPolygons(polygons, provinceId) {
  const safe = polygons.filter((polygon) => polygonIsSafe(polygon));
...
export function buildAnatoliaPhase2DAssets() {
  ...
  for (const site of sites) if (!isAnatoliaGeometryPoint(site.point) || !isPhysicalInteriorPoint(site.point)) throw new Error(`Invalid 1300 province anchor: ${site.provinceId} ${site.point.join(",")}`);
  ...
  return { schemaVersion: 1, geometryVersion: 11, historicalDate: "1300-01-01", provider: "historia-ai-curated-cartography", dataset: "anatolia-province-geometry-1300", projection: "EPSG:4326", method: "one historical province anchor per political cell, deterministic physical-anchor resolution, anchor-safe physical-land clipping, curated coastal corrections, explicit water exclusions, lake-safe validation, dense physical sampling", siteCount: physicalSamplingSiteCount + sites.length, politicalSiteCount: sites.length, physicalSamplingSiteCount, naturalFeatureSiteCount: physicalSamplingSiteCount, barrierSiteCount: 0, supportSiteCount: 0, fallbackProvinceCount: 0, provinceCount: provinces.length, polygonCount, weightIterations: solved.iterations, provinces, geometries, sites };
}
