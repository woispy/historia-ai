...
function politicalPoint(point) {
  const [lon, lat] = point;
  return lon >= BBOX[0] && lon <= BBOX[2] && lat >= BBOX[1] && lat <= BBOX[3] && isPhysicalLandPoint(point);
}
...
function addSite(sites, seen, point, provinceId, kind) {
  ...
  seen.add(key); sites.push({ point, provinceId, kind });
}
...
function addProvinceSites(sites, seen) {
  ...
  for (const item of ANATOLIA_PROVINCE_METADATA) {
    ...
    for (const radius of RADII) for (let direction = 0; direction < DIRECTIONS; direction += 1) {
      const angle = direction / DIRECTIONS * Math.PI * 2 + jitter(sequence++, item.id.length) * 0.1;
...
function addAdjacencySites(sites, seen) {
  ...
  for (const [id, neighbors] of Object.entries(ANATOLIA_ADJACENCY_HINTS)) {
    const a = province(id); if (!a) continue;
    for (const neighborId of neighbors) {
      const key = [id, neighborId].sort().join("|"); if (emitted.has(key)) continue; emitted.add(key);
      const b = province(neighborId); if (!b) continue;
      const aa = anchor(a); const bb = anchor(b);
      const midpoint = [(aa[0] + bb[0]) / 2, (aa[1] + bb[1]) / 2];
      const dx = bb[0] - aa[0]; const dy = bb[1] - aa[1]; const length = Math.hypot(dx, dy) || 1;
...
function nearestProvince(point) {
  let winner = ANATOLIA_PROVINCE_METADATA[0]; let best = Infinity;
  for (const item of ANATOLIA_PROVINCE_METADATA) { const d = sq(point, anchor(item)); if (d < best) { best = d; winner = item; } }
  return winner;
}
...
function addCoastSites(sites, seen) {
  for (const polygon of physicalLandPolygons()) for (let i = 0; i < polygon.length - 1; i += 1) {
    ...
    for (let step = 0; step < steps; step += 1) {
      const t = (step + 0.5) / steps; const midpoint = [a[0] + dx * t, a[1] + dy * t];
      const candidates = [[midpoint[0] + normal[0] * 0.05, midpoint[1] + normal[1] * 0.05], [midpoint[0] - normal[0] * 0.05, midpoint[1] - normal[1] * 0.05]];
...
function addNaturalSites(sites, seen) {
  for (const feature of [...ANATOLIA_STRATEGIC_PASSES, ...ANATOLIA_RIVER_CROSSINGS]) for (const id of feature.provinces ?? []) {
    const item = province(id); if (!item) continue; const a = anchor(item); const dx = a[0] - feature.coordinate[0]; const dy = a[1] - feature.coordinate[1]; const length = Math.hypot(dx, dy) || 1;
...
function halfPlane(polygon, a, b, c) {
  const out = []; if (!polygon.length) return out; const inside = (p) => a * p[0] + b * p[1] <= c + EPS;
  for (let i = 0; i < polygon.length; i += 1) {
    const current = polygon[i]; const next = polygon[(i + 1) % polygon.length]; const ci = inside(current); const ni = inside(next);
    if (ci && ni) out.push(next); else if (ci !== ni) { const cv = a * current[0] + b * current[1] - c; const nv = a * next[0] + b * next[1] - c; const t = Math.abs(cv - nv) < EPS ? 0 : cv / (cv - nv); out.push([current[0] + (next[0] - current[0]) * t, current[1] + (next[1] - current[1]) * t]); if (!ci && ni) out.push(next); }
  }
  return out;
}

function voronoiCell(index, sites) {
  const site = sites[index].point; let polygon = [[BBOX[0], BBOX[1]], [BBOX[2], BBOX[1]], [BBOX[2], BBOX[3]], [BBOX[0], BBOX[3]]];
  for (let other = 0; other < sites.length; other += 1) { if (other === index) continue; const p = sites[other].point; const a = 2 * (p[0] - site[0]); const b = 2 * (p[1] - site[1]); const c = p[0] ** 2 + p[1] ** 2 - site[0] ** 2 - site[1] ** 2; polygon = halfPlane(polygon, a, b, c); if (polygon.length < 3) return []; }
  return polygon;
}
...
function edgeOnLand(polygon) {
  for (let i = 0; i < polygon.length; i += 1) for (const fraction of [0, 0.25, 0.5, 0.75, 1]) {
    const a = polygon[i]; const b = polygon[(i + 1) % polygon.length]; const p = [a[0] + (b[0] - a[0]) * fraction, a[1] + (b[1] - a[1]) * fraction];
...
function shrinkToLand(cell, anchorPoint) {
  ...
  for (let iteration = 0; iteration < 48; iteration += 1) {
    if (result.length >= 3 && area(result) >= MIN_AREA && edgeOnLand(result)) return result;
...
export function buildAnatoliaPhase2DAssets() {
  validateManifest(); const sites = []; const seen = new Set();
  addProvinceSites(sites, seen); addAdjacencySites(sites, seen); addNaturalSites(sites, seen); addCoastSites(sites, seen);
  ...
  for (const item of ANATOLIA_PROVINCE_METADATA) {
    const polygons = byProvince[item.id]; if (!polygons.length) throw new Error(`Phase 2D produced no physical-land geometry for ${item.id}`);
  ...
  return { schemaVersion: 1, geometryVersion: 7, historicalDate: "1300-01-01", provider: "historia-ai-curated-cartography", dataset: "anatolia-province-geometry-1300", projection: "EPSG:4326", method: "historical anchors, adjacency controls, natural-feature controls and coastline-interior controls with physical-land authority and explicit lake exclusion", siteCount: sites.length, politicalSiteCount: sites.length, barrierSiteCount: 0, naturalFeatureSiteCount, fallbackProvinceCount: 0, provinceCount: provinces.length, polygonCount: geometries.reduce((sum, item) => sum + item.polygons.length, 0), provinces, geometries };
}
