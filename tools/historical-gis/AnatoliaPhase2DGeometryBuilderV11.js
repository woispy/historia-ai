...
import {
  getPhysicalLandPolygons,
  isPhysicalLandPoint,
  pointInPolygon,
  signedArea,
  polygonArea,
  distanceToSegment,
} from "./Phase2DPhysicalMask.js";
...
function resolvePhysicalAnchor(point) {
  ...
  for (let dx = -max; dx <= max; dx += 1) {
    for (let dy = -max; dy <= max; dy += 1) {
      ...
      if (!distance || distance > ANCHOR_SEARCH_RADIUS || distance >= bestDistance) continue;
      const candidate = [point[0] + dx * ANCHOR_SEARCH_STEP, point[1] + dy * ANCHOR_SEARCH_STEP];
      if (physicalInterior(candidate)) {
        best = candidate;
  ...
  for (const polygon of getPhysicalLandPolygons()) {
    ...
    for (const vertex of polygon) {
      ...
      for (const fraction of [0.02, 0.05, 0.1, 0.2]) {
        const candidate = [
          vertex[0] + (center[0] - vertex[0]) * fraction,
          vertex[1] + (center[1] - vertex[1]) * fraction,
        ];
        if (physicalInterior(candidate)) {
          const candidateDistance = Math.hypot(candidate[0] - point[0], candidate[1] - point[1]);
...
function powerCell(index, sites, weights) {
  ...
  for (let other = 0; other < sites.length; other += 1) {
    ...
    const next = halfPlane(
      polygon,
      2 * (otherPoint[0] - site[0]),
      2 * (otherPoint[1] - site[1]),
      otherPoint[0] ** 2 + otherPoint[1] ** 2 - site[0] ** 2 - site[1] ** 2 + ownWeight - otherWeight,
    );
...
function clipPolygon(subject, clip) {
  ...
  const boundary = signedArea(clip) < 0 ? [...clip].reverse() : clip;
...
function polygonIsSafe(polygon, anchor) {
  return polygon.length >= 3
    && polygonArea(polygon) >= MIN_AREA
    && pointInPolygon(anchor, polygon)
    && polygon.every((point) => isPhysicalLandPoint(point))
    && edgeOnPhysicalLand(polygon);
}

function anchorFallback(cell, anchor) {
  if (!physicalInterior(anchor) || !pointInPolygon(anchor, cell)) return [];
  for (const scale of FALLBACK_SCALES) {
    const candidate = cell.map((point) => [
      anchor[0] + (point[0] - anchor[0]) * scale,
      anchor[1] + (point[1] - anchor[1]) * scale,
    ]);
...
function clipCellToPhysicalLand(cell, anchor) {
  ...
  for (const land of getPhysicalLandPolygons()) {
    ...
    if (polygonIsSafe(clipped, anchor)) candidates.push(clipped);
  }
  if (candidates.length) return [candidates.sort((a, b) => polygonArea(b) - polygonArea(a))[0]];
...
function buildControlSites() {
  return ANATOLIA_PROVINCE_METADATA.map((item) => {
    ...
    return {
      point: resolvePhysicalAnchor(sourcePoint),
      sourcePoint,
      provinceId: item.id,
...
function headers(item, type) {
  return {
    assetType: type,
    assetVersion: 14,
    generator: "Historia AI Phase 2D Geometry Builder V11",
...
function provinceAsset(item, polygons) {
  return {
    ...
    historical: {
      ...
      precision: "anchor-safe-physical-partition",
...
function geometryAsset(item, polygons) {
  return {
    ...
    ownership: { countryId: item.countryId, ownerId: item.historicalControl.controllerAt1300 ?? item.countryId },
    geometry: { type: "MultiPolygon", coordinates: polygons.map((polygon) => [polygon]), polygons },
    polygons,
    historical: { sourceFeatureId: item.id, sourceFeatureIndex: null, historicalDate: "1300-01-01", borderPrecision: headers(item, "geometry").borderPrecision },
  };
}
...
function areaSummary(partition) {
  return ANATOLIA_PROVINCE_METADATA.map((item) => ({
    provinceId: item.id,
    area: (partition.get(item.id) ?? []).reduce((sum, polygon) => sum + polygonArea(polygon), 0),
  }));
}
...
function buildPartition(sites, weights) {
  ...
  for (let index = 0; index < sites.length; index += 1) {
    ...
    if (!polygons.length) throw new Error(`Phase 2D V11 could not construct physical geometry for ${site.provinceId}`);
...
function solveWeights(sites) {
  ...
  for (let iteration = 0; iteration < MAX_WEIGHT_ITERATIONS; iteration += 1) {
    ...
    for (const item of summary) {
      const adjustment = (medianArea - item.area) / Math.max(medianArea, EPS);
      weights[item.provinceId] += Math.max(-MAX_WEIGHT_STEP, Math.min(MAX_WEIGHT_STEP, adjustment));
    }
    const next = buildPartition(sites, weights);
    const converged = summary.every((item) => Math.abs(
      (next.get(item.provinceId) ?? []).reduce((sum, polygon) => sum + polygonArea(polygon), 0) - item.area,
    ) < medianArea * 0.005);
    partition = next;
...
export function buildAnatoliaPhase2DAssets() {
  ...
  for (const site of sites) {
    if (!isAnatoliaGeometryPoint(site.sourcePoint) || !physicalInterior(site.point)) {
  ...
  return {
    ...
    method: "one historical province anchor per political cell, canonical physical-land mask, deterministic interior anchor resolution, weighted physical-land intersection, anchor-preserving fallback, explicit water exclusions, lake-safe validation, dense physical sampling",
