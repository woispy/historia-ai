  ...
  for (let edge = 0; edge < boundary.length; edge += 1) {
    ...
    for (let i = 0; i < input.length; i += 1) {
      ...
      if (currentInside && nextInside) output.push(next);
        ...
        const currentValue = cross(a, b, current);
        const nextValue = cross(a, b, next);
        const t = Math.abs(currentValue - nextValue) < EPS ? 0 : currentValue / (currentValue - nextValue);
        output.push([
          current[0] + (next[0] - current[0]) * t,
          current[1] + (next[1] - current[1]) * t,
...
function clippedLandFragments(cell, anchor) {
  ...
  for (const land of getPhysicalLandPolygons()) {
    ...
    if (clipped.length < 3 || polygonArea(clipped) < MIN_AREA) continue;
    if (!pointInPolygon(anchor, clipped)) continue;
...
function resolvePhysicalAnchor(point) {
  ...
  const max = Math.ceil(SEARCH_RADIUS / SEARCH_STEP);
...
function provinceAsset(item, polygons) {
  return {
    ...
    historical: {
      ...
      precision: "anchor-safe-physical-partition-v12",
...
function buildPartition(sites, weights) {
  ...
  for (let index = 0; index < sites.length; index += 1) {
    ...
    const polygons = buildProvinceGeometry(cell, site.point, site.provinceId);
    partition.set(site.provinceId, polygons.map((polygon) => polygon.map(([x, y]) => [Number(x.toFixed(5)), Number(y.toFixed(5))])));
  }
  return partition;
}
