...
const MAX_ANCHOR_SNAP_DISTANCE = 1.1;
...
function buildPartition(sites, weights) {
  ...
  for (let index = 0; index < sites.length; index += 1) {
    const site = sites[index]; const cell = powerCell(index, sites, weights); if (!cell.length) throw new Error(`Phase 2D V12 empty power cell: ${site.provinceId}`);
    const polygon = clipCellToLand(cell, site.point)[0]; if (!polygon || !edgeOnPhysicalLand(polygon)) throw new Error(`Phase 2D V12 produced invalid physical-land geometry: ${site.provinceId}`);
...
function solveWeights(sites) {
  ...
  if (maxArea > medianArea * MAX_AREA_RATIO) throw new Error(`Phase 2D V12 could not bound province area ratio: max ${maxArea.toFixed(3)} vs median ${medianArea.toFixed(3)}`);
...
function headers(item, type) { return { assetType: type, assetVersion: 13, generator: "Historia AI Phase 2D Geometry Builder V12", provider: "historia-ai-curated-cartography", dataset: "anatolia-province-geometry-1300", historicalDate: "1300-01-01", borderPrecision: item.borderConfidence === "high" ? 3 : 2, sourceFeatureId: item.id, sourceFeatureIndex: null }; }
...
export function buildAnatoliaPhase2DAssets() {
  ...
  for (const item of ANATOLIA_PROVINCE_METADATA) {
    const site = sites.find((candidate) => candidate.provinceId === item.id); const polygon = solved.partition.get(item.id); if (!polygon) throw new Error(`Phase 2D V12 produced no geometry for ${item.id}`);
