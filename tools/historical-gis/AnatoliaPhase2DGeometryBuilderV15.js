...
import { PHYSICAL_LAND_POLYGONS, isPhysicalLandPoint, isPhysicalGeometryBoundaryPoint, resolveGeometryAnchor, resolvePhysicalGeometryBoundaryPoint } from "./recovery/physical-land-authority.mjs";
...
function powerCell(index, sites, weights) {
  ...
  for (let otherIndex = 0; otherIndex < sites.length; otherIndex += 1) {
    ...
    const other = sites[otherIndex].point; const otherWeight = weights[sites[otherIndex].provinceId] ?? 0;
    polygon = halfPlane(polygon, 2 * (other[0] - site[0]), 2 * (other[1] - site[1]), other[0] ** 2 + other[1] ** 2 - site[0] ** 2 - site[1] ** 2 + ownWeight - otherWeight);
...
function clipLandByCell(land, cell) {
  ...
  for (let edge = 0; edge < clip.length; edge += 1) {
    ...
    for (let index = 0; index < input.length; index += 1) {
      const current = input[index]; const next = input[(index + 1) % input.length];
      const currentInside = inside(current); const nextInside = inside(next);
      if (currentInside && nextInside) output.push(next);
      else if (currentInside !== nextInside) {
        const currentValue = cross(start, end, current); const nextValue = cross(start, end, next);
        const denominator = currentValue - nextValue; const t = Math.abs(denominator) < EPS ? 0 : currentValue / denominator;
        output.push([current[0] + (next[0] - current[0]) * t, current[1] + (next[1] - current[1]) * t]);
        if (!currentInside && nextInside) output.push(next);
      }
    }
...
function clipCellToLand(cell, anchorPoint) {
  const candidates = PHYSICAL_LAND_POLYGONS.map((land) => clipLandByCell(land, cell))
    .filter((polygon) => polygon.length >= 3 && area(polygon) >= MIN_AREA)
    .filter((polygon) => edgeOnPhysicalLand(polygon));
...
function normalizePhysicalBoundary(polygon) {
  ...
  for (const point of polygon) {
    const resolved = resolvePhysicalGeometryBoundaryPoint(point); if (!resolved) return null;
    const last = normalized[normalized.length - 1];
    if (!last || last[0] !== resolved[0] || last[1] !== resolved[1]) normalized.push(resolved);
  }
  if (normalized.length > 1) {
    const first = normalized[0]; const last = normalized[normalized.length - 1];
...
function buildPartition(sites, weights) {
  ...
  for (let index = 0; index < sites.length; index += 1) {
    ...
    const rawPolygon = clipCellToLand(cell, site.point)[0]; const polygon = rawPolygon ? normalizePhysicalBoundary(rawPolygon) : null;
...
function solveWeights(sites) {
  ...
  if (maxArea > medianArea * MAX_AREA_RATIO) throw new Error(`Phase 2D V15 could not bound province area ratio: max ${maxArea.toFixed(3)} vs median ${medianArea.toFixed(3)}`);
...
function headers(item, type) { return { assetType: type, assetVersion: 16, generator: "Historia AI Phase 2D Geometry Builder V15", provider: "historia-ai-curated-cartography", dataset: "anatolia-province-geometry-1300", historicalDate: "1300-01-01", borderPrecision: item.borderConfidence === "high" ? 3 : 2, sourceFeatureId: item.id, sourceFeatureIndex: null }; }
...
export function buildAnatoliaPhase2DAssets() {
  validateManifest(); const controlSites = buildControlSites();
  for (const site of controlSites) if (!isAnatoliaGeometryPoint(site.point) || !isPhysicalLandPoint(site.point)) throw new Error(`Invalid snapped 1300 province anchor: ${site.provinceId} ${site.point.join(",")}`);
  const solved = solveWeights(controlSites); const provinces = []; const geometries = [];
  for (const item of ANATOLIA_PROVINCE_METADATA) {
    const site = controlSites.find((candidate) => candidate.provinceId === item.id); const polygon = solved.partition.get(item.id);
...
export function isAnatoliaGeometryPoint(point) { const [longitude, latitude] = point; return longitude >= BBOX[0] && longitude <= BBOX[2] && latitude >= BBOX[1] && latitude <= BBOX[3]; }
