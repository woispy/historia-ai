...
function sample(start, end, fraction) { return [start[0] + (end[0] - start[0]) * fraction, start[1] + (end[1] - start[1]) * fraction]; }
function isValidPhysicalPoint(point) { return isPhysicalLandPoint(point) || isFinalPhysicalGeometryBoundaryPoint(point); }
...
function lakeRings(lake) { return normalizeRings(lake?.rings ?? lake?.coordinates); }
...
function segmentBounds(start, end) {
  return { minX: Math.min(start[0], end[0]), minY: Math.min(start[1], end[1]), maxX: Math.max(start[0], end[0]), maxY: Math.max(start[1], end[1]) };
}
function cellCoordinate(value) { return Math.floor(value / INDEX_CELL_SIZE); }
...
function addToGrid(grid, segment) {
  ...
  for (let x = minX; x <= maxX; x += 1) for (let y = minY; y <= maxY; y += 1) {
    ...
    if (bucket) bucket.push(segment); else grid.set(key, [segment]);
  }
}
function buildBoundarySegments() {
  ...
  const appendBoundary = (boundary, kind, metadata = {}) => {
    ...
    for (let segmentIndex = 0; segmentIndex < boundary.length; segmentIndex += 1) {
      ...
      segments.push(segment); addToGrid(grid, segment);
    }
  };
...
function intersections(start, end, boundaries, kind) {
  ...
  for (const descriptor of BOUNDARY_INDEX.segments) {
    ...
    const duplicate = result.some((entry) => Math.abs(entry.fraction - fraction) <= EPS && entry.boundary === descriptor.boundary && entry.segmentIndex === descriptor.segmentIndex);
...
function locateTransition(start, end, validFraction, invalidFraction) {
  ...
  for (let iteration = 0; iteration < BINARY_ITERATIONS; iteration += 1) {
    ...
    if (isValidPhysicalPoint(sample(start, end, midpoint))) valid = midpoint; else invalid = midpoint;
  }
...
function boundaryArcCandidates(boundary, from, to) {
  ...
  const forward = [from.point]; let index = (from.segmentIndex + 1) % count; let guard = 0;
  while (index !== (to.segmentIndex + 1) % count && guard <= count) { forward.push(boundary[index]); index = (index + 1) % count; guard += 1; }
  forward.push(to.point);
  const backward = [from.point]; index = from.segmentIndex; guard = 0;
  while (index !== to.segmentIndex && guard <= count) { backward.push(boundary[index]); index = (index - 1 + count) % count; guard += 1; }
...
function recoverDegenerateWaterEdge(start, end) {
  ...
  for (const from of startCandidates) for (const to of endCandidates) {
    ...
    if (isValidPhysicalPath(path)) candidates.push({ path, score: pathLength(path) + from.distance + to.distance });
  }
  candidates.sort((a, b) => a.score - b.score);
  if (candidates.length) return candidates[0].path;
  const resolvedStart = resolveBoundary(start); const resolvedEnd = resolveBoundary(end);
...
function waterIntervals(start, end) {
  const states = [];
  for (let index = 0; index <= SAMPLE_COUNT; index += 1) states.push({ fraction: index / SAMPLE_COUNT, valid: isValidPhysicalPoint(sample(start, end, index / SAMPLE_COUNT)) });
  const intervals = [];
  for (let index = 1; index < states.length; index += 1) {
    const previous = states[index - 1]; const current = states[index];
...
function recoveryCandidates(interval, start, end, lakeCrossings, landCrossings) {
  ...
  for (const entry of landEntries) for (const exit of landExits) {
    if (entry.boundary !== exit.boundary || distance(entry.point, exit.point) <= GEOMETRY_EPS) continue;
    const arc = chooseBoundaryArc(entry.boundary, entry, exit);
    if (arc) result.push({ path: arc, score: pathLength(arc) + entry.distance + exit.distance });
  }
...
function repairPhysicalEdge(start, end) {
  ...
  for (let intervalIndex = 0; intervalIndex < intervals.length; intervalIndex += 1) {
    ...
    if (!selected) {
      ...
      if (!entry || !exit || !isValidPhysicalPath([cursor, entry]) || !isValidPhysicalPath([exit, nextTransition])) {
        throw new Error(`Physical water crossing has no valid boundary recovery path: ${start.join(",")} -> ${end.join(",")}`);
      }
      const fallback = [entry, exit];
      if (!isValidPhysicalPath(fallback)) throw new Error(`Physical water crossing has invalid fallback boundary path: ${start.join(",")} -> ${end.join(",")}`);
    ...
    if (!isValidPhysicalPath([cursor, pathStart]) || !isValidPhysicalPath([pathEnd, nextTransition])) {
      throw new Error(`Physical water crossing has no valid boundary recovery path: ${start.join(",")} -> ${end.join(",")}`);
    }
...
export function repairPhysicalPolygon(polygon) {
  ...
  for (let index = 0; index < normalizedPolygon.length; index += 1) {
    const start = normalizedPolygon[index]; const end = normalizedPolygon[(index + 1) % normalizedPolygon.length];
