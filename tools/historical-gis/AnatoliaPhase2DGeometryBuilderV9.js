...
function recoverLandConstrainedCell(cell, anchor, radiusScale = 1) {
  ...
  const points = []; const maxRadius = RECOVERY_MAX_RADIUS * radiusScale;
  for (let index = 0; index < RECOVERY_RAYS; index += 1) {
    const angle = (index / RECOVERY_RAYS) * Math.PI * 2; const direction = [Math.cos(angle), Math.sin(angle)]; let low = 0; let high = maxRadius;
...
function clipCellToMainland(cell, provinceId, anchor) {
  ...
  for (let step = 0; step <= RECOVERY_SCALE_STEPS; step += 1) {
    const scale = 1 - step / RECOVERY_SCALE_STEPS; const recovered = recoverLandConstrainedCell(cell, anchor, scale);
