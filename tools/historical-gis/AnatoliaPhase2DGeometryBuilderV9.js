...
import {
  ANATOLIA_PROVINCE_METADATA_44,
  ANATOLIA_PROVINCE_BY_ID_44,
} from "../../src/map/data/AnatoliaProvinceMetadata44.js";
import {
...
const province = (id) => ANATOLIA_PROVINCE_BY_ID_44[id] ?? null;
...
function powerCell(index, sites, weights) {
  ...
  for (let other = 0; other < sites.length; other += 1) {
    ...
    polygon = halfPlane(
      ...
      2 * (p[0] - site[0]),
      2 * (p[1] - site[1]),
      p[0] ** 2 + p[1] ** 2 - site[0] ** 2 - site[1] ** 2 + ownWeight - otherWeight,
    );
...
function clipCellToMainland(cell, provinceId) {
  ...
  if (polygons.length !== 1) {
    throw new Error(`Phase 2D V9 ${provinceId} must resolve to one contiguous mainland polygon; got ${polygons.length}.`);
  }
  return polygons[0];
}

function buildControlSites() {
  return ANATOLIA_PROVINCE_METADATA_44.map((item) => ({
...
function validateManifest() {
  ...
  if (ANATOLIA_1300_PROVINCE_GEOMETRY_MANIFEST_44.length !== ids.size) {
    throw new Error("44-province geometry manifest is not aligned with 44-province metadata.");
  }
  for (const entry of ANATOLIA_1300_PROVINCE_GEOMETRY_MANIFEST_44) {
    if (!ids.has(entry.id) || !ANATOLIA_1300_PROVINCE_GEOMETRY_KEYS_44[entry.id] || entry.clipToPhysicalLand !== true) {
      throw new Error(`Invalid 44-province geometry manifest entry: ${entry.id}`);
    }
  }
}
...
function solveWeights(sites) {
  ...
  if (maxArea > medianArea * MAX_AREA_RATIO) {
    throw new Error(`Phase 2D V9 could not bound province area ratio: max ${maxArea.toFixed(3)} vs median ${medianArea.toFixed(3)}`);
  }
...
export function buildAnatoliaPhase2DAssets() {
  ...
  for (const item of ANATOLIA_PROVINCE_METADATA_44) {
    ...
    provinces.push(provinceAsset(item, polygon.map(([x, y]) => [Number(x.toFixed(5)), Number(y.toFixed(5))])));
    geometries.push(geometryAsset(item, polygon.map(([x, y]) => [Number(x.toFixed(5)), Number(y.toFixed(5))])));
  }
