...
function polygonCentroid(polygon) {
  ...
  for (let i = 0; i < polygon.length; i += 1) { const a = polygon[i]; const b = polygon[(i + 1) % polygon.length]; const cross = a[0] * b[1] - b[0] * a[1]; twiceArea += cross; x += (a[0] + b[0]) * cross; y += (a[1] + b[1]) * cross; }
...
function halfPlane(polygon, a, b, c) {
  ...
  for (let i = 0; i < polygon.length; i += 1) {
    const current = polygon[i]; const next = polygon[(i + 1) % polygon.length]; const currentInside = inside(current); const nextInside = inside(next);
    if (currentInside && nextInside) output.push(next);
      ...
      output.push([current[0] + (next[0] - current[0]) * t, current[1] + (next[1] - current[1]) * t]); if (!currentInside && nextInside) output.push(next);
    }
  }
  return output;
}
...
function clipLandByCell(land, cell) {
  ...
  for (let edge = 0; edge < clip.length; edge += 1) {
    ...
    for (let i = 0; i < input.length; i += 1) {
      const current = input[i]; const next = input[(i + 1) % input.length]; const currentInside = inside(current); const nextInside = inside(next);
      if (currentInside && nextInside) output.push(next);
      else if (currentInside !== nextInside) {
...
function recoverLandConstrainedCell(cell, anchor) {
  ...
  for (let index = 0; index < RECOVERY_RAYS; index += 1) {
    const angle = (index / RECOVERY_RAYS) * Math.PI * 2; const direction = [Math.cos(angle), Math.sin(angle)]; let low = 0; let high = RECOVERY_MAX_RADIUS;
    for (let iteration = 0; iteration < RECOVERY_BISECTIONS; iteration += 1) {
      const radius = (low + high) / 2; const sample = [center[0] + direction[0] * radius, center[1] + direction[1] * radius];
...
function clipCellToMainland(cell, provinceId, anchor) {
  const polygons = ANATOLIA_PHYSICAL_ATLAS.landPolygons.filter((polygon) => area(polygon) >= MAINLAND_MIN_AREA)
    .map((land) => clipLandByCell(land, cell)).filter((polygon) => polygon.length >= 3 && area(polygon) >= MIN_AREA).sort((a, b) => area(b) - area(a));
...
function validateAnchors(sites) {
  for (const site of sites) if (!isAnatoliaGeometryPoint(site.point) || inLake(site.point) || distanceToLand(site.point) > ANCHOR_COAST_TOLERANCE) throw new Error(`Invalid 1300 province anchor: ${site.provinceId} ${site.point.join(",")}`);
}
function buildPartition(sites, weights) {
  const partition = new Map();
  for (let index = 0; index < sites.length; index += 1) { const site = sites[index]; const cell = powerCell(index, sites, weights); if (!cell.length) throw new Error(`Phase 2D V9 empty power cell: ${site.provinceId}`); partition.set(site.provinceId, clipCellToMainland(cell, site.provinceId, site.point)); }
  return partition;
}
...
function solveWeights(sites) {
  ...
  for (let iteration = 0; iteration < CENTROID_MAX_ITERATIONS; iteration += 1) {
    const failures = centroidFailures(partition); if (!failures.length) break; for (const provinceId of failures) weights[provinceId] -= CENTROID_WEIGHT_STEP; partition = buildPartition(sites, weights);
  }
...
export function buildAnatoliaPhase2DAssets() {
  ...
  const physicalSamplingSiteCount = samplingSiteCount(); const provinceCount = provinces.length; const polygonCount = geometries.reduce((sum, item) => sum + item.polygons.length, 0);
  return { schemaVersion: 1, geometryVersion: 12, historicalDate: "1300-01-01", provider: "historia-ai-curated-cartography", dataset: "anatolia-province-geometry-1300", projection: "EPSG:4326", method: "44 historical province identities partitioned by one historical anchor per province using a deterministic weighted power diagram, clipped to the single Anatolian mainland physical mask; exactly one contiguous polygon per province and no support fragments", siteCount: physicalSamplingSiteCount + sites.length + naturalFeatureSiteCount, politicalSiteCount: sites.length, physicalSamplingSiteCount, barrierSiteCount: 0, naturalFeatureSiteCount, supportSiteCount: 0, fallbackProvinceCount: 0, provinceCount, polygonCount, weightIterations: solved.iterations, provinces, geometries };
}
