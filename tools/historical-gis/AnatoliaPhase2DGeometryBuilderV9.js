...
function isStaticLandPoint(point) {
  if (coreLandPolygons().some((polygon) => pointInPolygon(point, polygon))) return true;
  return coreLandPolygons().some((polygon) => distanceToPolygon(point, polygon) <= COAST_TOLERANCE);
}
...
function halfPlane(polygon, a, b, c) {
  ...
  for (let i = 0; i < polygon.length; i += 1) {
    ...
    const ci = inside(current); const ni = inside(next);
...
function clipCellToLand(cell, anchorPoint) {
  const candidates = coreLandPolygons().map((land) => clipLandByCell(land, cell)).filter((polygon) => polygon.length >= 3 && area(polygon) >= MIN_AREA);
  if (!candidates.length) return [];
  const containing = candidates.filter((polygon) => pointInPolygon(anchorPoint, polygon));
  return [([...(containing.length ? containing : candidates)].sort((a, b) => area(b) - area(a))[0])];
}
...
function lakesInside(polygon) {
  return ANATOLIA_PHYSICAL_ATLAS_RUNTIME.lakes.filter((lake) => pointInPolygon(lake.coordinates?.[0] ?? [Infinity, Infinity], polygon));
}
...
function buildPartition(sites, weights) {
  ...
  for (let index = 0; index < sites.length; index += 1) {
    ...
    if (!polygon || !edgeOnPhysicalLand(polygon)) throw new Error(`Phase 2D V9 produced invalid physical-land geometry: ${site.provinceId}`);
    map.set(site.provinceId, polygon.map(([x, y]) => [Number(x.toFixed(5)), Number(y.toFixed(5))]));
  }
  return map;
}
...
function provinceAsset(item, polygon, holes) { return { header: headers(item, "province"), identity: { id: item.id, name: item.name }, references: { geometryId: item.id, countryId: item.countryId, capitalCityId: item.cityId }, ownership: { countryId: item.countryId, ownerId: item.historicalControl.controllerAt1300 ?? item.countryId }, historical: { sourceFeatureId: item.id, sourceFeatureIndex: null, sourceName: item.name, subject: item.countryId, partOf: item.regionId, borderPrecision: headers(item, "province").borderPrecision, classification: "phase2d-anatolia-province-geometry", precision: "single-anchor-weighted-land-partition-with-lake-holes", anchor: rawAnchor(item), historicalControl: item.historicalControl }, geometry: { coastal: item.coastal, port: item.port, terrain: item.terrain, strategic: item.strategic }, polygons: [polygon], holes } }
function geometryAsset(item, polygon, holes) { return { header: headers(item, "geometry"), identity: { id: item.id, provinceId: item.id }, metadata: { sourceFeatureId: item.id, sourceFeatureIndex: null, name: item.name, subject: item.countryId, partOf: item.regionId, borderPrecision: headers(item, "geometry").borderPrecision, classification: "phase2d-anatolia-province-geometry", precision: "single-anchor-weighted-land-partition-with-lake-holes", anchor: rawAnchor(item) }, polygons: [polygon], holes } }
...
export function buildAnatoliaPhase2DAssets() {
  ...
  for (const item of ANATOLIA_PROVINCE_METADATA) {
    const polygon = solved.partition.get(item.id); if (!polygon) throw new Error(`Phase 2D V9 produced no geometry for ${item.id}`);
    const holes = ANATOLIA_PHYSICAL_ATLAS_RUNTIME.lakes.filter((lake) => pointInPolygon(lake.coordinates?.[0] ?? [Infinity, Infinity], polygon)).map((lake) => lake.coordinates);
    provinces.push(provinceAsset(item, polygon, holes)); geometries.push(geometryAsset(item, polygon, holes));
  }
  ...
  return { schemaVersion: 1, geometryVersion: 12, historicalDate: "1300-01-01", provider: "historia-ai-curated-cartography", dataset: "anatolia-province-geometry-1300", projection: "EPSG:4326", method: "38 historical province identities, one anchor per province, deterministic weighted power partition, physical-land clipping, explicit lake holes, no support-control fragments", siteCount: samplingSiteCount() + sites.length + naturalFeatureSiteCount, politicalSiteCount: sites.length, physicalSamplingSiteCount: samplingSiteCount(), barrierSiteCount: 0, naturalFeatureSiteCount, supportSiteCount: 0, fallbackProvinceCount: 0, provinceCount: provinces.length, polygonCount: geometries.length, weightIterations: solved.iterations, provinces, geometries };
}
