...
assert.ok(sourcePath, "A2_V15_SOURCE_FILE is required");
assert.ok(sourceSHA, "A2_V15_SOURCE_SHA is required");
...
function instrument(sourceText) {
  ...
  s = s.replace("const rawAnchor = (item) => ANATOLIA_PROVINCE_REFINEMENTS[item.id]?.anchor ?? item.centroid;", "const TARGET_PROVINCE_ID = \"pontus-amisos\";\nconst rawAnchor = (item) => ANATOLIA_PROVINCE_REFINEMENTS[item.id]?.anchor ?? item.centroid;");
  s = s.replace("export { isPhysicalLandPoint };", "export { isPhysicalLandPoint, __A2_RECORDS };");
  return s;
}

const tempPath = path.resolve(`.a2-v15-source-${process.pid}.mjs`);
...
try {
  ...
  const report = { migrationId: "MIG-2.8-A2", targetProvince: TARGET, sourcePath, sourceSHA, telemetryOnly: true, mutation: false, rawArea: raw?.area ?? null, normalizedArea: normalized?.postDedupArea ?? null, minArea: normalized?.minArea ?? null, areaRatioToMinArea: raw?.area != null && normalized?.minArea ? raw.area / normalized.minArea : null, collapseObserved: Boolean(raw?.area != null && normalized?.postDedupArea != null && normalized.postDedupArea < (normalized.minArea ?? 0)), repairFailureCount: repairFailures.length, edgeRepairFailureCount: edgeFailures.length, edgeRecords: records.filter((record) => record.stage === "normalize-edge" || record.stage === "normalize-edge-result" || record.stage === "normalize-edge-repair-failure"), repairRecords: records.filter((record) => record.stage.startsWith("repair-")), records };
