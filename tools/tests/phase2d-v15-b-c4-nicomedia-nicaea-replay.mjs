...
async function instrumentV15() {
  ...
  const injected = source.replace(marker, "export { powerCell, repairPhysicalEdge, normalizePhysicalBoundary, edgeOnPhysicalLand };\n\n" + marker);
...
const rows = FAILURE_EDGES.map((edge) => {
  ...
  const supportInvalid = edgeInvalids(edge, (p) => v15.isPhysicalGeometryBoundaryPoint ? v15.isPhysicalGeometryBoundaryPoint(p) : true);
