...
assert.equal(source.geometryPointCount, 9);
...
assert.deepEqual(Array.from(source.geometryView(4, 5)), [-5,-4,-1,-4,-1,0,-5,0,-5,-4]);
assert.deepEqual(Array.from(source.tileRecord(0)), [0,4,0,0,0,0]);
assert.deepEqual(Array.from(source.tileRecord(1)), [4,5,1,0,0,0]);
