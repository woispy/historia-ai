...
assert.match(host,/loadMapBin\(assetUrl\)/);assert.match(host,/BinaryMapRenderer/);assert.doesNotMatch(host,/MapAssetBridge/);assert.doesNotMatch(host,/GpuMapRenderer/);assert.doesNotMatch(host,/onPointerMove/);assert.match(loader,/BinaryMapAssetSource\.fromArrayBuffer/);
...
console.log("Map rendering contract passed: production WebGL2 renderer implements the hover contract, uses real triangle-list geometry, and maps picking coordinates through the active camera.");
