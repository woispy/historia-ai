...
assert.match(gpuLayer, /if \(texture\(uLandMask, vUv\)\.r < 0\.5\) discard;/);
assert.match(gpuLayer, /if \(provinceId < 0\.5\) discard;/);
assert.doesNotMatch(gpuLayer, /uWaterColor/);
assert.doesNotMatch(gpuLayer, /uLandColor/);
assert.match(gpuLayer, /createTexture\(gl, raster\.landCanvas, false\)/);
...
assert.doesNotMatch(cartographyLayer, /ANATOLIA_STRATEGIC_CORRIDORS/);
assert.doesNotMatch(cartographyLayer, /ANATOLIA_STRATEGIC_PASSES/);
assert.doesNotMatch(cartographyLayer, /ANATOLIA_STRATEGIC_CROSSINGS/);
...
assert.doesNotMatch(cityLayer, /strokeDasharray/);
assert.doesNotMatch(cityLayer, /port &&/);
assert.doesNotMatch(cityLayer, /fortified &&/);
