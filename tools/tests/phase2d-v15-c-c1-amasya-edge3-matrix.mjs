...
const { pointInPolygon, isLakeInteriorPoint, nearestLakeBoundaryPoint, nearestBoundaryLandPoint, isPhysicalLandPoint, isPhysicalGeometryBoundaryPoint, isFinalPhysicalGeometryBoundaryPoint, resolvePhysicalGeometryBoundaryPoint } = authority;
...
for (let i = 0; i <= SAMPLE_COUNT; i += 1) {
  ...
  samples.push({ fraction, point, lakeInterior: isLakeInteriorPoint(point), lakeIds: lakeIds(point), lakeBoundary: ANATOLIA_PHYSICAL_ATLAS_RUNTIME.lakes.some((lake) => lakeBoundary(point, lake)), physicalLand: isPhysicalLandPoint(point), geometryBoundary: isPhysicalGeometryBoundaryPoint(point), finalBoundary: isFinalPhysicalGeometryBoundaryPoint(point), nearestLakeBoundary: shoreline.point, nearestLakeBoundaryDistance: shoreline.distance, nearestLandBoundary: land.point, nearestLandBoundaryDistance: land.distance, shorelineCloser: shoreline.distance <= land.distance, resolver, resolverDistance: dist(point,resolver) });
}
...
const resolverCompetition = samples.filter((s) => s.lakeInterior).map((s) => ({ fraction: s.fraction, point: s.point, shorelineDistance: s.nearestLakeBoundaryDistance, landDistance: s.nearestLandBoundaryDistance, shorelineCloser: s.shorelineCloser, resolver: s.resolver }));

console.log(JSON.stringify({ phase: "C1 — Pontus-Amasya Edge 3 authority decision matrix", edge: { start: START, end: END }, sampleCount: SAMPLE_COUNT, firstInvalid, counts: { samples: samples.length, interiorSamples: interiorSamples.length, resolverFailures: resolverFailures.length, shorelineResolutions: shorelineResolutions.length }, endpointStart: samples[0], endpointEnd: samples.at(-1), resolverCompetition, samples }, null, 2));
