...
try {
  ...
  const invalidThrow = /if\(!polygon\|\|!polygon\.every\(isPhysicalGeometryBoundaryPoint\)\)throw new Error\(`Phase 2D V16 produced invalid physical-land geometry: \$\{site\.provinceId\}`\);/;
  assert.ok(invalidThrow.test(original), "historical invalid-geometry throw marker missing");
  let patched = original.replace(currentLand, oldLand);
  // Forensic isolation only: skip unrelated province failures so Amisos can be observed.
  patched = patched.replace(invalidThrow, "if(!polygon||!polygon.every(isPhysicalGeometryBoundaryPoint)){continue;}");
