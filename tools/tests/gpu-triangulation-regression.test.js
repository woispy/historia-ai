...
import { triangulateRing, normalizeRing } from "../../src/map/rendering/gpu/ProvinceGpuPack.js";
...
const cases = [
  ...
  [[0, 0], [2, 0], [4, 0.00000000001], [4, 2], [2, 2], [0, 2]],
];
for (const ring of cases) {
  const normalized = normalizeRing(ring);
  const triangles = triangulateRing(ring, { provinceId: "regression" });
  assert.equal(triangles.length, (normalized.length - 2) * 3);
  assert.ok(triangles.every((index) => Number.isInteger(index) && index >= 0 && index < normalized.length));
  assert.ok(Math.abs(triangleArea(normalized, triangles) - area(normalized)) < 1e-8);
}
