...
const canonicalModule=await import(`file://${path.join(CANONICAL_ROOT,"tools/historical-gis/AnatoliaPhase2DGeometryBuilder.js")}?b4-authority=${process.pid}`);
...
const matrix=FAILURE_EDGES.map((edge)=>{const canonicalHit=findEdge(edge,canonicalRecords);const v15Hit=findEdge(edge,v15Records);const canonicalAuthority=firstInvalid(edge,canonicalModule.isPhysicalLandPoint);const v15Authority=firstInvalid(edge,authority.isPhysicalGeometryBoundaryPoint);return{...edge,canonicalHit,v15Hit,calibratedPowerCellMatch:Boolean(v15Hit),calibratedSameOwnerSeed:Boolean(canonicalHit&&v15Hit&&canonicalHit.siteIndex===v15Hit.siteIndex),canonicalFirstInvalid:canonicalAuthority,v15FirstInvalid:v15Authority,authorityDivergence:Boolean(canonicalAuthority)!==Boolean(v15Authority),endpointAuthority:{start:{canonical:canonicalModule.isPhysicalLandPoint(edge.start),v15:authority.isPhysicalGeometryBoundaryPoint(edge.start)},end:{canonical:canonicalModule.isPhysicalLandPoint(edge.end),v15:authority.isPhysicalGeometryBoundaryPoint(edge.end)}}};});
