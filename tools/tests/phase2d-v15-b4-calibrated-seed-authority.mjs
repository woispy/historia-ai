...
async function loadCanonicalInstrumented() {
  ...
  instrumented = instrumented.replace(
    ...
    "export { isPhysicalLandPoint, __B4, buildVoronoiCell };",
  );
...
try { canonical.buildAnatoliaPhase2DAssets(); } catch {}
...
assert.ok(sites.length > 0, "Canonical site universe was not captured");

const v15Records = [];
for (let siteIndex = 0; siteIndex < sites.length; siteIndex += 1) {
  const cell = v15.powerCell(siteIndex, sites, Object.fromEntries(sites.map((site) => [site.provinceId, 0])));
...
const matrix = FAILURE_EDGES.map((edge) => {
  ...
  const sameSeed = canonicalHit && v15Hit && canonicalHit.siteIndex === v15Hit.siteIndex;
  return {
    ...edge,
    canonicalHit,
    v15Hit,
    calibratedPowerCellMatch: Boolean(v15Hit),
    calibratedSameOwnerSeed: Boolean(sameSeed),
