...
for (const zoom of [1, 1.5, 2, 3, 4, 8, 16]) {
  ...
  for (let index = 0; index < labels.length; index += 1) {
    ...
    const leftBox = {
      ...
      top: left.y - left.effectiveFontSize * 0.78,
      bottom: left.y + left.effectiveFontSize * 0.24,
    };
    for (let rightIndex = index + 1; rightIndex < labels.length; rightIndex += 1) {
      ...
      const rightBox = {
        ...
        top: right.y - right.effectiveFontSize * 0.78,
        bottom: right.y + right.effectiveFontSize * 0.24,
      };
  ...
  assert.deepEqual(
    labels.map(({ city, x, y, anchor, fontSize, screenScale }) => [city.id, x, y, anchor, fontSize, screenScale]),
    secondPass.map(({ city, x, y, anchor, fontSize, screenScale }) => [city.id, x, y, anchor, fontSize, screenScale]),
  );

  const capital = cities.find((city) => city.map.tier === "capital");
  assert.ok(getCityVisualStyle(capital, zoom).fontSize >= 0.045);
  assert.ok(getCityVisualStyle(capital, zoom).screenSize > 0);
  assert.ok(getCityMarkerBudget(zoom) >= getCityLabelBudget(zoom));
}

// fontSize is world-space authoring data. screenSize is the actual rendered
// size after the nested inverse-zoom transform and must remain stable.
const capital = cities.find((city) => city.map.tier === "capital");
...
for (const label of focusedLabels) {
  ...
  const top = label.y - label.effectiveFontSize * 0.78;
  const bottom = label.y + label.effectiveFontSize * 0.24;
