...
class MockRenderer extends MapRendererContract {
  ...
  setHoveredRasterId(id) { this.hovered.push(id); }
  lookupRasterId(id) { return id === "province-7" ? 7 : 0; }
...
try {
  ...
  const rig = new MockCameraRig();
  const controller = new MapRuntimeController({
    canvas,
    cameraRig: rig,
    renderer,
  });
  ...
  // A burst of pointer movement before the next frame must collapse to one
  // pick, and the latest pointer coordinate must win.
  ...
  assert.deepEqual(renderer.hovered, [7]);

  // A second frame is allowed to sample exactly once again.
  controller.queueHover(70, 80);
  controller.queueHover(90, 100);
  flushRaf();
  assert.deepEqual(renderer.picks, [[50, 60], [90, 100]]);

  // Camera updates are imperative and do not depend on React renders.
