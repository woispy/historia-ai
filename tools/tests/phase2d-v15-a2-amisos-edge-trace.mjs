...
traced = traced.replace(
  ...
  "function repairPhysicalEdge(start, end, depth = 0) {\n  const repairId = __a2Trace.active ? __a2Trace.repairs.length : -1;\n  if (__a2Trace.active) __a2Trace.repairs.push({ repairId, depth, start: __a2Point(start), end: __a2Point(end), inputArea: __a2Area([start, end, end]), physicalBefore: edgeIsPhysical(start, end) });\n  if (edgeIsPhysical(start, end)) { __a2Event(\"repair-accepted\", { repairId, depth }); return [start, end]; }"
);
traced = traced.replace(
  "  if (depth >= MAX_EDGE_REPAIR_DEPTH) return null;",
  "  if (depth >= MAX_EDGE_REPAIR_DEPTH) { __a2Event(\"repair-depth-limit\", { repairId, depth }); return null; }"
);
traced = traced.replace(
  "  if (invalidFraction === null) return [start, end];",
  "  if (invalidFraction === null) { __a2Event(\"repair-no-invalid-sample\", { repairId, depth }); return [start, end]; }"
);
traced = traced.replace(
...
traced = traced.replace(
  ...
  "    const repaired = repairPhysicalEdge(resolvedStart, resolvedEnd);\n    __a2Event(\"edge-result\", { edgeIndex: index, start: __a2Point(start), end: __a2Point(end), resolvedStart: __a2Point(resolvedStart), resolvedEnd: __a2Point(resolvedEnd), repairedVertexCount: repaired?.length ?? 0, repairedAreaProxy: repaired ? __a2Area(repaired.length >= 3 ? repaired : [resolvedStart, resolvedEnd, resolvedEnd]) : null });\n    if (!repaired) return null;\n    normalized.push(...repaired.slice(0, -1));"
);
traced = traced.replace(
...
try {
  ...
  const mod = await import(`${moduleUrl}?a2=${Date.now()}`);
  const result = mod.buildAnatoliaPhase2DAssets([]);
  const trace = mod.__getA2Trace();
  assert.equal(result.geometries.length, 38);
  assert.equal(trace.provinceId, "pontus-amisos");
  const amisos = result.geometries.find((geometry) => geometry.identity.provinceId === "pontus-amisos");
  assert.ok(amisos, "Amisos geometry must exist");
  const finalPolygon = amisos.polygons[0];
  ...
  const raw = events.find((event) => event.type === "raw-polygon");
  const normalized = events.find((event) => event.type === "normalize-result");
  const serialized = events.find((event) => event.type === "serialized");
  assert.ok(raw, "A2 raw-polygon trace event missing");
  assert.ok(normalized, "A2 normalize-result trace event missing");
  assert.ok(serialized, "A2 serialized trace event missing");
  const report = {
    provinceId: "pontus-amisos",
    raw: { vertexCount: raw.vertexCount, area: raw.rawArea },
    normalized: { vertexCount: normalized.normalizedVertexCount, area: normalized.normalizedArea, meetsMinArea: normalized.meetsMinArea },
    serialized: { vertexCount: serialized.vertexCount, area: serialized.area },
    finalGeometry: { vertexCount: finalPolygon.length, area: area(finalPolygon) },
