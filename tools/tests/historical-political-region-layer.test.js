...
const boundarySource = read("src/map/rendering/historical/HistoricalPoliticalBoundary.js");
const worldMapSource = read("src/map/components/WorldMap.jsx");
...
assert.match(layerSource, /stroke=\{color\}/, "coastal political expansion must use the owning historical polity colour");
assert.match(layerSource, /buildCartographicInternalBoundaryPath\(provinces\)/, "historical political layer must render the shared cartographic boundary geometry");
assert.doesNotMatch(layerSource, /HISTORICAL_POLITICAL_CARTOGRAPHIC_FILTER_ID|feTurbulence|feDisplacementMap/, "historical province borders must not rely on noisy SVG displacement filters");
...
assert.match(provinceLayerSource, /renderBoundaries = true/, "ProvinceLayer must expose an explicit boundary-rendering switch");
assert.match(provinceLayerSource, /renderBoundaries && <ProvinceBoundaryLayer/, "ProvinceLayer must conditionally render the legacy straight topology");
...
const runtimeProvinces = ANATOLIA_PROVINCE_METADATA.map((metadata) => ({ id: metadata.id, owner: metadata.countryId ?? null }));
const politicalModel = createHistoricalPoliticalMapModel({ date: "1300-01-01", provinces: runtimeProvinces, countryRepository: { byId: {} } });
assert.equal(politicalModel.length, ANATOLIA_PROVINCE_METADATA.length, "the 1300 political presentation must cover every curated Anatolia province");
assert.equal(politicalModel.every((entry) => entry.historicalPolitical?.id), true, "every province must have a visible historical political presentation, including neutral areas");
assert.equal(politicalModel.every((entry) => /^#[0-9a-f]{6}$/i.test(entry.historicalPolitical?.color ?? "")), true, "every province presentation must provide an explicit political colour");
...
assert.equal(getHistoricalPoliticalOverlayMode(byProvince.get("cappadocia-kayseri")), "suzerainty");
assert.equal(getHistoricalPoliticalOverlayMode(byProvince.get("phrygia-eskisehir")), "contested");
assert.equal(getHistoricalPoliticalOverlayMode(byProvince.get("ionia-ayasuluk")), "sovereign");
...
assert.equal(buildCartographicInternalBoundaryPath(sameProvinceGeometry), "", "seams between polygons belonging to the same province must remain invisible");
assert.equal(historicalAtlas.regions.some((region) => region.id === "anatolia_aydin_pre1308" && region.countryId === "byzantium"), true, "Aydinid ownership must not be projected backward into 1300");
assert.equal(historicalAtlas.regions.some((region) => region.countryId === "inanc"), true, "İnanç Beyliği must remain represented in the research atlas");
assert.equal(historicalAtlas.regions.some((region) => region.countryId === "hamid"), true, "Hamid Beyliği must remain represented in the research atlas");
assert.equal(historicalAtlas.regions.some((region) => region.countryId === "sahibata"), true, "Sâhib Ata Beyliği must remain represented in the research atlas");

console.log(`Historical political region layer tests passed: ${politicalModel.length} rendered province presentations plus ${historicalAtlas.regions.length} research regions.`);
