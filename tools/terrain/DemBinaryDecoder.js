...
function parseGeoTiff(buffer) {
  ...
  const rowsPerStrip = fields.get(278);
  if (!width || !height || !bits) throw new Error("GeoTIFF requires width, height and bits-per-sample tags.");
  if (width.count !== 1 || height.count !== 1 || bits.count !== 1) throw new Error("GeoTIFF DEM dimensions and bits-per-sample must be scalar values.");
  ...
  const rowsPerStripValue = rowsPerStrip ? readValue(view, rowsPerStrip.offset, rowsPerStrip.type, little) : heightValue;
