...
assert.equal(adjacency[1].neighbors.south.mode,"same");
assert.equal(adjacency[2].neighbors.north.mode,"same");
...
assert.throws(
  ()=>toTerrainDrawPlanAdjacency(adjacency[0],[{tile:tiles[0],neighbors:{north:null,east:{tileIndex:1},south:null,west:null},edgeSegments:{north:[],east:[{tileIndex:1,overlapStart:0,overlapEnd:1}],south:[],west:[]}}]),
  /missing tile/
);
